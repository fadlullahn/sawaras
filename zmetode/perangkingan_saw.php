...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="font-bold text-xl leading-6 text-gray-900">1. Matriks Keputusan</h1>
        </div>
    </div>
    <table class="mt-8 min-w-full divide-y divide-gray-200 border border-gray-300">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" colspan="<?php echo count($criteria_data); ?>">Kriteria</th>
            </tr>
        </thead>
        <tbody class="bg-white divide-y divide-gray-200">
            <?php foreach ($decision_matrix as $iddaftar => $criteria_values) : ?>
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap"><?= $alternatives[$iddaftar] ?></td>
                    <?php foreach ($criteria_values as $data) : ?>
...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="font-bold text-xl leading-6 text-gray-900">2. Matriks Normalisasi</h1>
        </div>
    </div>
    <table class="mt-8 min-w-full divide-y divide-gray-200 border border-gray-300">
        <thead class="bg-gray-50">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            <?php foreach ($normalized_matrix as $i => $normalized_values) : ?>
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap"><?php echo $iu++ ?></td>
                    <td class="px-6 py-4 whitespace-nowrap"><?= $alternatives[$i] ?></td>
                    <?php foreach ($normalized_values as $j => $normalized_value) : ?>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <b><?= $criteria_data[$j][0] ?></b>
                            <br>
                            <?= $normalized_value ?>
                        </td>
                    <?php endforeach; ?>
...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="font-bold text-xl leading-6 text-gray-900">3. Perangkingan SAW</h1>
        </div>
    </div>
